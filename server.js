    ...
    try {
      const preference = await new Preference(mercadopago).create({
        body: {
          ...
          back_urls: {
            success: "https://www.consultavehicular.services/reult2.html",
...
async function consultarPapeletasHuanuco(placa) {
  ...
  try {
    ...
    if (!multasHtml || 
        multasHtml.includes("no registra multas") || 
        multasHtml.trim() === "") {

      return {
        tienePapeletas: false
      };
    }
...
async function consultarPapeletasChachapoyas(placa) {
  ...
  try {
    ...
    await page.goto(
      'https://app.munichachapoyas.gob.pe/servicios/consulta_papeletas/app/papeletas.php',
      { waitUntil: 'domcontentloaded' }
    );
    ...
    const resultado = await page.evaluate(() => {
      ...
      const texto = cont.innerText.toLowerCase();

      if (texto.includes('no se encontraron') || texto.includes('no tiene')) {
        return { tienePapeletas: false };
      }

      const tabla = cont.querySelector('table');
      if (!tabla) return { tienePapeletas: false };
 ...
 async function consultarPapeletasCajamarca(placa) {
  ...
  try {
    ...
    const datos = await page.evaluate(() => {
      ...
      }).filter(p => p.papeleta && p.papeleta.trim() !== '' && p.papeleta.trim() !== '0.00');
    });
...
async function consultarPapeletasCusco(placa) {
  ...
  try {
    ...
    await page.goto('https://cusco.gob.pe/informatica/index.php/', { 
      waitUntil: 'networkidle2' 
    });
...
async function consultarPapeletasIca(placa) {
  ...
  try {
    ...
    // ✅ Detectar si no tiene papeletas
    const noTiene = await page.evaluate(() => {
      return document.body.innerText.includes("no cuenta con papeletas");
    });
...
async function resolverCapt(imageBase64) {
  ...
  const API_KEY = "d6fb31ad4bee4d576b69ceacc98c0b25"; // tu key
  ...
  if (response.data.status !== 1) {
    throw new Error("Error enviando captcha a 2captcha");
  }

  const captchaId = response.data.request;

  // Esperar respuesta
  for (let i = 0; i < 24; i++) { // 2 minutos máximo
    await new Promise(r => setTimeout(r, 5000));
...
async function consultarAndahuaylas(placa) {
  ...
  try {
    ...
    await page.goto(
      "https://muniandahuaylas.gob.pe/consultar-papeleta/",
      { waitUntil: "domcontentloaded", timeout: 30000 }
    );

    await page.waitForSelector("#nro_placa", { timeout: 10000 });
    await page.type("#nro_placa", placa);

    /* ===== CAPTCHA ===== */
    await page.waitForSelector("#captcha img", { timeout: 10000 });

    const imageElement = await page.$("#captcha img");
    if (!imageElement) {
      throw new Error("No se encontró la imagen captcha");
    }

    const imageBuffer = await imageElement.screenshot();
    const base64Captcha = imageBuffer.toString("base64");
    ...
    if (
      contenido.toLowerCase().includes("no se encontraron resultados")
    ) {
      await browser.close();
      return {
        success: true,
        html: `<p style="color:green;font-weight:bold;">
                ✅ La placa ${placa} no tiene papeletas registradas.
               </p>`
      };
    }
    ...
    return {
      success: false,
      error: "No se encontró tabla ni mensaje de resultados"
    };
...
async function consultarPapeletasPuno(placa) {
  ...
  const browser = await puppeteer.launch({
    headless: true, // 🔥 en producción debe ir true
    args: ['--no-sandbox', '--disable-setuid-sandbox']
  });
  ...
  try {
    ...
    await page.goto('https://papeletas.munipuno.gob.pe/', {
      waitUntil: 'networkidle2'
    });

    await page.waitForSelector('input[placeholder*="Ingrese una placa"]', { timeout: 10000 });

    // Limpiar input
    await page.click('input[placeholder*="Ingrese una placa"]', { clickCount: 3 });
    await page.type('input[placeholder*="Ingrese una placa"]', placa);

    // Click en botón Buscar por texto
    await page.evaluate(() => {
      const botones = Array.from(document.querySelectorAll('button'));
      const btnBuscar = botones.find(b => b.innerText.includes('Buscar'));
    ...
    const resultado = await page.evaluate(() => {
      ...
      // 🔴 Mensaje: No existe esa placa
      const mensajeError = Array.from(document.querySelectorAll('div'))
        .find(div => div.innerText.includes('No existe esa placa en el sistema'));
      ...
      const data = filas.map(fila =>
        Array.from(fila.querySelectorAll('td, th')).map(cell =>
          cell.textContent.trim()
        )
      );
...
async function consultarPapeletasTacna(placa) {
  ...
  try {
    ...
    await page.goto(
      "https://www.munitacna.gob.pe/pagina/sf/servicios/papeletas",
    ...
    const resultado = await page.evaluate(() => {
      ...
      // 🟢 NO HAY PAPELETAS
      if (!tabla && mensaje &&
          mensaje.innerText.toUpperCase().includes("NO SE ENCONTRARON DATOS")) {
        return { tienePapeletas: false };
      }
...
module.exports = consultarPapeletasTacna;
...
async function resolverCaptcha(sitekey, pageurl) {
  ...
  const { data: enviar } = await axios.get(
    "http://2captcha.com/in.php",
    {
      params: {
        key: API_KEY,
  ...
  if (enviar.status !== 1) {
    throw new Error("Error enviando captcha: " + enviar.request);
  }

  const captchaId = enviar.request;
  ...
  while (true) {
    ...
    if (respuesta.status === 1) return respuesta.request;

    if (respuesta.request !== "CAPCHA_NOT_READY") {
      throw new Error("Error resolviendo captcha: " + respuesta.request);
    }
  }
}
...
/* ============================
   RUTA PIT INTEGRADA
============================ */
app.post("/consultarPIT", async (req, res) => {

  const { placa } = req.body;

  if (!placa)
    return res.json({ error: "Placa requerida" });
  ...
  try {
    ...
    const PAGE_URL = "http://www.pit.gob.pe/pit2007/EstadoCuenta.aspx";

    await page.goto(PAGE_URL, { waitUntil: "domcontentloaded" });

    // Click menú velocidad
    await page.click("#UcMenuIzquierdo1_dtlEnlaces_ctl02_lnkMenuEnlaces");
    ...
    const token = await resolverCaptcha(sitekey, page.url());
    ...
    // Ingresar placa
    await page.click("#txtPlaca", { clickCount: 3 });
    await page.keyboard.press("Backspace");
    await page.type("#txtPlaca", placa.toUpperCase(), { delay: 80 });

    await page.click("#btnBuscarPlaca");

    await page.waitForTimeout(5000);

    // Verificar mensaje vacío
    const mensaje = await page.$eval(
      "#lblMensajeVacio",
    ...
    const resultados = await page.evaluate(() => {
      ...
      const filas = Array.from(
        document.querySelectorAll("#grdEstadoCuenta tbody tr")
      ).filter(tr => tr.classList.contains("celda3"));

      return filas.map(fila => {

        const celdas = fila.querySelectorAll("td");
        const link = fila.querySelector("a")?.href || null;

        return {
          placa: celdas[0]?.innerText.trim(),
          documento: celdas[1]?.innerText.trim(),
          fecha: celdas[2]?.innerText.trim(),
          total: celdas[3]?.innerText.trim(),
          estado: celdas[4]?.innerText.trim(),
          falta: celdas[5]?.innerText.trim(),
          licencia: celdas[6]?.innerText.trim(),
          evidencia: link
        };
      });
    });
...
// 🔹 piura
  ...
  app.post("/consultar-lunas", async (req, res) => {
    const { placa } = req.body;
    ...
    try {
        ...
        await page.goto("https://www.permisodelunaspolarizado.com/", {
            waitUntil: "networkidle2"
        });

        await page.type("#placa", placa.toUpperCase());

        await page.click("#submitBtn");

        // 🔥 Esperar que cambie el contenido dentro del main
        await page.waitForSelector("main", { timeout: 15000 });

        // Esperar que aparezca el mensaje de resultado
        await page.waitForFunction(() => {
            const main = document.querySelector("main");
            return main && main.innerText.includes("Permiso");
        }, { timeout: 15000 });

        const resultadoHTML = await page.$eval("main", el => el.outerHTML);
        ...
        await browser.close();
        res.status(500).json({ error: "Error al consultar lunas" });
    }
});
 ...
 app.post("/generar-reporte", (req, res) => {
  ...
  try {
    ...
    const placa = datos.placa || "SIN-PLACA";
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Reporte_${placa}.pdf`
    );

    doc.pipe(res);

    // ===== TÍTULO =====
    doc.fontSize(18).text("REPORTE VEHICULAR OFICIAL", { align: "center" });
    doc.moveDown();

    doc.fontSize(10)
      .text(`Placa: ${placa}`)
      .text(`Fecha: ${new Date().toLocaleString()}`)
      .text(`Código: RV-${Date.now()}`);

    doc.moveDown(2);

    // ===== FUNCIÓN TABLA =====
    function dibujarTabla(titulo, data) {
      ...
      headers.forEach((header, i) => {
        ...
        doc.text(header.toUpperCase(), 45 + i * colWidth, y + 5, {
          width: colWidth - 10
        });
      });
      ...
      data.forEach(row => {
        ...
        headers.forEach((header, i) => {
          doc.rect(40 + i * colWidth, y, colWidth, 20).stroke();
          doc.text(String(row[header] || ""), 45 + i * colWidth, y + 5, {
            width: colWidth - 10
          });
        });
    ...
    // ===== SECCIONES =====
    dibujarTabla("SAT LIMA", datos.lima);
    dibujarTabla("SAT CALLAO", datos.callao);

    doc.fontSize(14).text("SUTRAN", { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(10).text(datos.sutran || "Sin información");
    doc.moveDown();

    doc.fontSize(14).text("SBS - SINIESTROS", { underline: true });
    doc.moveDown(0.5);
    doc.fontSize(10).text(datos.sbs || "Sin información");
